...
export function registerBadLog(context, qType, qName): string {
  ...
  if (context.user.isAuth === undefined) {
    ...
    logAction('Unauthenticated', 'Authentication', qType, qName, mensaje, context.user.ip);
    return mensaje;
  } else {
    mensaje = `Requested the action without sufficient permissions`;
    logAction(context.user.userId, 'Authentication', qType, qName, mensaje, context.user.ip)
  }
  return mensaje;
}
