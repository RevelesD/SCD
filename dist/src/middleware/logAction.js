...
exports.logAction = async (causer, type, queryType, queryName, description, ip) => {
    // console.log(causer);
    try {
...
function registerBadLog(context, qType, qName) {
    ...
    if (context.user.isAuth === undefined) {
        ...
        exports.logAction('Unauthenticated', 'Authentication', qType, qName, mensaje, context.user.ip);
        return mensaje;
    }
    else {
        mensaje = `Requested the action without sufficient permissions`;
        exports.logAction(context.user.userId, 'Authentication', qType, qName, mensaje, context.user.ip);
    }
    return mensaje;
}
...
function registerGoodLog(context, qType, qName, oid) {
    ...
    // console.log(context);
    exports.logAction(context.user.userId, 'Success', qType, qName, `Query resolved successfully on ${oid}`, context.user.ip);
}
exports.registerGoodLog = registerGoodLog;
function registerErrorLog(context, qType, qName, err) {
    // console.log('Error Log');
    exports.logAction(context.user.userId, 'Error', qType, qName, err, context.user.ip);
}
exports.registerErrorLog = registerErrorLog;
function registerGenericLog(context, qType, qName, message) {
    exports.logAction(context.user.userId, 'Generic', qType, qName, message, context.user.ip);
}
